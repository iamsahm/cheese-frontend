...
describe("Cheeselist displays cheeseslistitems components", () => {
    it("displays the short cheese details for the given cheese id", () => {
        ...
        cy.mount(<CheeseList cheeseType={"hard"} />);
        cy.wait("@getCheese");

        cy.get("h1").should("contain", "Hard Cheeses");
        cy.get("ul")
            .first()
            .should("contain", "Cheddar")
            .and(
                "contain",
                "A hard, sharp cheese with big chunky crystals made in Somerset in England. It is a hard and natural ..."
            )
            .and("contain", "Somerset: England");
        cy.get("ul")
            .last()
            .should("contain", "Gruyere")
            .and(
                "contain",
                "A hard yellow cheese made from cows milk, named after the town of Gruyères in Switzerland. It is swe..."
            )
            .and("contain", "Gruyere: Switzerland")
            .and("have.length", 2);
    });
});
